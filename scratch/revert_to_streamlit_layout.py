...
# I will define the old text block exactly as it exists in the file now.
# Since I used `st.html(html)` instead of `st.markdown`, I will search for the end of the `html` string block and the `st.html(html)` call.
...
if idx2 != -1:
    idx2 += len("        except Exception as e:\n            st.error(f\"Error parsing report: {e}\")")
