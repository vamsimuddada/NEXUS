        ...
        if data.get("type") == "PLANNER_RESPONSE":
            for tc in data.get("tool_calls", []):
                if tc["name"] == "view_file" and "dashboard.py" in str(tc["args"]):
                    # If StartLine is not in args, it reads up to 800 lines!
                    if "StartLine" not in tc["args"]:
                        for j in range(i+1, min(i+5, len(lines_list))):
                            ...
                            if next_data.get("source") == "SYSTEM" and next_data.get("type") == "GENERIC":
                                ...
                                if len(content) > 1000:
                                    with codecs.open(f"scratch/dashboard_view_{i}.txt", "w", encoding="utf-8") as out:
                                        out.write(content)
                                    print(f"Extracted view from step {i}")
